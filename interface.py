...
objPlayback = {
    ...
    "strMode": "automatic",
    "intTime": 0,
    "fltTime": numpy.linspace(0.0, 1.0, 75).tolist()
    + list(reversed(numpy.linspace(0.0, 1.0, 75).tolist())),
    "strCache": {},
    "objFrom": {
        "fltCenterU": 512.0,
        "fltCenterV": 384.0,
...
objFlask = flask.Flask(
    import_name=__name__, static_url_path="", static_folder=os.path.abspath("./")
)
...
def load_image():
    ...
    objPlayback["npyImage"] = numpy.ascontiguousarray(
        cv2.imdecode(
            buf=numpy.fromstring(
                base64.b64decode(flask.request.form["strData"].split(";base64,")[1]),
                numpy.uint8,
            ),
            flags=-1,
        )[:, :, 0:3]
...
def autozoom():
    ...
    objPlayback["objTo"] = process_autozoom(
        {"fltShift": 100.0, "fltZoom": 1.25, "objFrom": objPlayback["objFrom"]}
    )

    return flask.jsonify(
        {"objFrom": objPlayback["objFrom"], "objTo": objPlayback["objTo"]}
    )
...
def get_live():
    def generator():
        ...
        while True:
            for intYield in range(100):
                gevent.sleep(0.0)

            gevent.sleep(max(0.0, (1.0 / 25.0) - (time.time() - fltFramelimiter)))
            fltFramelimiter = time.time()

            if objPlayback["strImage"] is None:
                yield b"--frame\r\nContent-Type: image/jpeg\r\n\r\n" + cv2.imencode(
                    ext=".jpg",
                    img=numpy.ones([768, 1024, 3], numpy.uint8) * 29,
            ...
            if str(fltTime) not in objPlayback["strCache"]:
                ...
                objPlayback["strCache"][str(fltTime)] = (
                    b"--frame\r\nContent-Type: image/jpeg\r\n\r\n"
                    + cv2.imencode(
                        ext=".jpg",
                        img=npyKenburns,
...
def get_result():
    ...
    moviepy.editor.ImageSequenceClip(
        sequence=[
            npyFrame[:, :, ::-1]
            for npyFrame in npyKenburns + list(reversed(npyKenburns))[1:]
        ],
        fps=25,
    ).write_videofile(strTempdir + "/kenburns.mp4")
    ...
    return flask.send_file(
        filename_or_fp=objKenburns,
        mimetype="video/mp4",
        as_attachment=True,
        attachment_filename="kenburns.mp4",
        cache_timeout=-1,
    )
